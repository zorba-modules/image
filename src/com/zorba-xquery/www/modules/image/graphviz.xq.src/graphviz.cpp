...
GraphvizFunction::getGraphvizTmpFileName() {
  ...
  {
	  throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( "Test"));
  }
  ...
  while (lChildren->next(lItem)) {
    ...
    if (lNodeName.getLocalName().equals(lAttrQName.getLocalName())) {
      ...
      if (!getAttribute(aFactory, "name", lItem, lNameAttr)) {
		  throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( "GXL parse error: attr node does not have a name attribute"));
      }
      ...
      if (!getAttribute(aFactory, "href", lItem, lHRefAttr)) {
		throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS("GXL parse error: type node does not have a href attribute"));
    ...
    const Item& in, std::fstream& os)
{
  ...
  if (!getAttribute(aFactory, "id", in, lItem)) {
	throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS("GXL parse error: node does not have an id attribute"));
  }
    ...
    const Item& in, std::fstream& os)
{
  ...
  if (!getAttribute(aFactory, "id", in, lIdAttr)) {
	throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( "GXL parse error: edge does not have an id attribute"));
  }
    ...
    const Item& in, std::fstream& os)
{
  ...
  if (!getAttribute(aFactory, "id", in, lGraphId)) {
	throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( "GXL parse error: graph does not have an id attribute"));
  }
    ...
    const Item& in, std::fstream& os)
{
  ...
  if (!in.isNode()) {
	throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( "GXL parse error: item is not a node"));
  }
  ...
  if (!lNodeName.getLocalName().equals(lGXLQName.getLocalName())) {
    ...
    lErrorMsg << "GXL parse error: only element with name "
              ...
              << ").";
	throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( lErrorMsg.str()));
  }
...
DotFunction::LazyDotSequence::InternalIterator::next(Item& aItem)
{
  ...
  try {
    ...
    if (!lGraph) {
	  throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS( "could not read input"));
    }

    if ( gvLayout(lGvc, lGraph, const_cast<char*>("dot")) != 0 ) {
	  throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS("could not generate layout"));
    }
    ...
    if ( gvRenderFilename(lGvc, lGraph, const_cast<char*>("svg"),
	  ...
	  throw XQUERY_EXCEPTION_VAR(zerr::ZXQP0001_DYNAMIC_RUNTIME_ERROR, ERROR_PARAMS("could not render graph"));
    }
